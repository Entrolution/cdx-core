...
pub trait Verifier {
    ...
    /// # Errors
    ///
    /// Returns an error if verification fails due to an error (not just invalid signature).
    fn verify(
        &self,
...
impl EcdsaSigner {
    ...
    pub fn from_pem(pem: &str, signer_info: SignerInfo) -> Result<Self> {
        ...
        let signing_key = p256::ecdsa::SigningKey::from_pkcs8_pem(pem)
            .map_err(|e| signature_error(format!("Failed to parse private key PEM: {e}")))?;
    ...
    pub fn generate(signer_info: SignerInfo) -> Result<(Self, String)> {
        ...
        let public_key_pem = verifying_key
            ...
            .map_err(|e| signature_error(format!("Failed to encode public key: {e}")))?;
...
impl Signer for EcdsaSigner {
    ...
    fn sign(&self, document_id: &DocumentId) -> Result<Signature> {
        ...
        let signature: p256::ecdsa::Signature = self.signing_key.sign(document_id.digest());
...
impl EcdsaVerifier {
    ...
    pub fn from_pem(pem: &str) -> Result<Self> {
        ...
        let verifying_key = p256::ecdsa::VerifyingKey::from_public_key_pem(pem)
            .map_err(|e| signature_error(format!("Failed to parse public key PEM: {e}")))?;
    ...
    fn verify(
        ...
        if signature.algorithm != SignatureAlgorithm::ES256 {
            return Ok(SignatureVerification::invalid(
                &signature.id,
                format!(
                    "Algorithm mismatch: expected ES256, got {}",
                    signature.algorithm
                ),
            ));
        }
        ...
        let ecdsa_sig = p256::ecdsa::Signature::from_slice(&sig_bytes)
            .map_err(|e| signature_error(format!("Invalid signature format: {e}")))?;
...
mod tests {
    ...
    fn generate_keypair() -> (EcdsaSigner, EcdsaVerifier) {
        let signer_info = SignerInfo::new("Test Signer");
        let (signer, public_key_pem) = EcdsaSigner::generate(signer_info).unwrap();
        let verifier = EcdsaVerifier::from_pem(&public_key_pem).unwrap();
        (signer, verifier)
    }
    ...
    fn test_generate_and_sign() {
        ...
        test_helpers::assert_sign_produces_valid_signature(&signer, SignatureAlgorithm::ES256);
    }
